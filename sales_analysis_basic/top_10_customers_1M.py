...
top_products = sales.groupby(["customer", "product"])["revenue"].sum().reset_index()

top_3_products_per_customer = top_products.sort_values(["customer", "revenue"], ascending=[True, False]).groupby("customer").head(3)
